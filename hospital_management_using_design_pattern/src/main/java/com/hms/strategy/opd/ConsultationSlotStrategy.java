...
public class ConsultationSlotStrategy implements SlotCreationStrategy {
        ...
        switch (request.getSlotType()) {
            case DAILY -> {
                for (LocalDate date = request.getStartDate(); !date.isAfter(request.getEndDate()); date = date.plusDays(1)) {
                    slots.addAll(generateSlotsForDay(doctor.getId(), date, startTime, endTime, duration));
                }
            }
            case WEEKLY -> {
                for (LocalDate date = request.getStartDate(); !date.isAfter(request.getEndDate()); date = date.plusDays(1)) {
    ...
    private List<TimeSlot> generateSlotsForDay(String doctorId, LocalDate date,
